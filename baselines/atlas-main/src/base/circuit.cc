  ...
  for (auto &schedule : schedules) {
    ...
    for (auto &kernel : schedule.kernels) {
      ...
      if (kernel.type == quartz::KernelType::fusion) {
        ...
        printf("compile: processing kernel gates, count = %d\n", (int)kernel.gates->gates.size());
        for(auto &gate : kernel.gates->gates) {
          ...
          for (const auto &input_wire : gate->input_wires) {
            if (input_wire->is_qubit()) {
              ...
              // 添加调试信息
              printf("Param %d, ", input_wire->index);
              printf("ctx pointer: %p, ", ctx);
              if (ctx != nullptr) {
                printf("param_info pointer: %p, ", ctx->get_param_info());
                // 检查参数是否是常量
                if (ctx->param_is_const(input_wire->index)) {
                  params.push_back(ctx->get_param_value(input_wire->index));
                } else {
                  // 如果参数不是常量，使用随机参数值
                  params.push_back(quartz::Rational(0));
                }
              } else {
                // 如果 ctx 是 nullptr，使用默认值 0.0
                printf("ctx is nullptr, ");
                params.push_back(quartz::Rational(0));
              }
            }
          }
...
bool qcircuit::Circuit<DT>::FuseGates(const quartz::Kernel &kernel,
  ...
  for (int i = 0; i < kernel.gates->gates.size(); i++) {
      printf("Gate %d: [", i);
      printf("kernel.gates = %p, ", kernel.gates.get());
      printf("kernel.gates->gates.size() = %d, ", (int)kernel.gates->gates.size());
      printf("kernel.gates->gates[%d] = %p, ", i, kernel.gates->gates[i].get());
      std::vector<int> qubit_indices;
      std::vector<ParamType> params;
      printf("input_wires.size() = %d, ", (int)kernel.gates->gates[i]->input_wires.size());
      printf("gate = %p, ", kernel.gates->gates[i]->gate);
      for (const auto &input_wire : kernel.gates->gates[i]->input_wires) {
        printf("input_wire = %p, ", input_wire);
        printf("is_qubit = %d, ", input_wire->is_qubit());
        if (input_wire->is_qubit()) {
          qubit_indices.push_back(input_wire->index);
          ...
          printf("Param %d, ", input_wire->index);
          printf("ctx pointer: %p, ", ctx);
          // 暂时使用默认值 0，避免调用 ctx->get_param_value
          printf("Using default param value 0, ");
          params.push_back(quartz::Rational(0));
        }
      }
      printf("]\n");
    printf("Before getMat_per_device loop, n_devices = %d\n", n_devices);
    for (int d = 0; d < n_devices; d++) {
      printf("d = %d, myRank = %d, calling getMat_per_device\n", d, myRank);
      unsigned mask = 0;
      ...
      if (ctx != nullptr) {
        ...
        printf("ctx is nullptr, skipping getMat_per_device\n");
      }
      //otherwise skip
    } 
  }

  printf("After loop, before Gate creation\n");
  printf("g_type = %d, n_target = %d, n_control = %d\n", g_type, n_target, n_control);
  printf("target size = %d, control size = %d\n", (int)target.size(), (int)control.size());
  printf("res_mats size = %d\n", (int)res_mats.size());
  ...
  printf("FuseGates returning true\n");
  return true;
}
...
bool qcircuit::Circuit<DT>::getMat_per_device(quartz::Context *ctx, int part_id, quartz::Gate* gate, std::vector<int> qubit_indices, std::vector<ParamType>& params, std::vector<ComplexType>& res, unsigned &mask, std::vector<int>& perm) {
  printf("Fusing gate %d(%d) for rank %d, params size: %d\n", gate->tp, gate->get_num_control_qubits(), part_id, (int)params.size());
  if (gate->get_num_control_qubits() == 2) { //ccx
    ...
    if (local_mask[c1] && local_mask[c2]) { // CCU(c1, c2, t)
      // 检查 qubit 是否在 qubit_group_map_fusion 中
      if (qubit_group_map_fusion.find(c1) == qubit_group_map_fusion.end() || 
          qubit_group_map_fusion.find(c2) == qubit_group_map_fusion.end() || 
          qubit_group_map_fusion.find(t) == qubit_group_map_fusion.end()) {
        printf("Warning: Qubit not found in qubit_group_map_fusion, skipping gate\n");
        return false;
      }
      ...
      for (int t = 0; t < qubit_indices.size(); t++) {
        auto it = qubit_group_map_fusion.find(qubit_indices[t]);
        if (it != qubit_group_map_fusion.end()) {
          int v = it->second;
      ...
      return true;
        ...
        if (IS_HIGH_PART(part_id, c1) && IS_HIGH_PART(part_id, c2)) { // U(t)
          // 检查 qubit 是否在 qubit_group_map_fusion 中
          if (qubit_group_map_fusion.find(t) == qubit_group_map_fusion.end()) {
            printf("Warning: Qubit not found in qubit_group_map_fusion, skipping gate\n");
            return false;
          }
          if(gate->tp == quartz::GateType::ccx) {
    ...
    if (local_mask[c] && local_mask[t]) { // CU(c, t)
      // 检查 qubit 是否在 qubit_group_map_fusion 中
      if (qubit_group_map_fusion.find(c) == qubit_group_map_fusion.end() || 
          qubit_group_map_fusion.find(t) == qubit_group_map_fusion.end()) {
        printf("Warning: Qubit not found in qubit_group_map_fusion, skipping gate\n");
        return false;
      }
