...
void QuantumComputation::import(const std::string& filename, Format format) {
  ...
  // 移除 ./ 前缀
  if (temp_filename.substr(0, 2) == "./") {
    temp_filename = temp_filename.substr(2);
  }
  
  const std::size_t slash = temp_filename.find_last_of('/');
  const std::size_t dot = temp_filename.find_last_of('.');
  
  if (slash != std::string::npos) {
    name = temp_filename.substr(slash + 1, dot - slash - 1);
  } else {
    // 没有斜杠，直接从开始提取到点
    name = temp_filename.substr(0, dot);
  }
