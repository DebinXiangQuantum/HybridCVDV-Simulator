...
void check_and_create_input_batch_file(int num_qubits) {
    // 构建 input_batch 文件路径
...
public:
    ...
    qc(std::move(qc_)), batch_size(batch_size_), num_batch(num_batch_) 
    {
        ...
        // 检查并创建对应的 input_batch 文件
        check_and_create_input_batch_file(nQubits);
    ...
    void singleShot(bool ignoreNonUnitaries) {
        ...
        std::cout << "传输时间: " << transfer_time_ << " ms" << std::endl;
        std::cout << "计算时间: " << computation_time_ << " ms" << std::endl;
        std::cout << "峰值内存使用: " << peak_memory_usage_ << " 字节" << std::endl;
    }
