...
public:
    ...
    virtual void apply(GPUState& state, int num_qubits, int num_qumodes, int cutoff) const = 0;
...
public:
    ...
    void apply(GPUState& state, int num_qubits, int num_qumodes, int cutoff) const override;
...
private:
    int qubit;
    int qumode;
    std::complex<double> displacement_param;
public:
    ConditionalDisplacementGate(int q, int qm, std::complex<double> param) 
...
private:
    int qumode;
    std::complex<double> squeezing_param;
public:
    SqueezingGate(int qm, std::complex<double> param) : qumode(qm), squeezing_param(param) {}
    void apply(GPUState& state, int num_qubits, int num_qumodes, int cutoff) const override;
...
private:
    ...
    std::complex<double> displacement_param;
public:
    DisplacementGate(int qm, std::complex<double> param) : qumode(qm), displacement_param(param) {}
    void apply(GPUState& state, int num_qubits, int num_qumodes, int cutoff) const override;
...
public:
    ...
    static std::unique_ptr<Gate> ConditionalDisplacement(int qubit, int qumode, std::complex<double> displacement_param) {
        return std::make_unique<::gpu::ConditionalDisplacementGate>(qubit, qumode, displacement_param);
    }
    
    static std::unique_ptr<Gate> Squeezing(int qumode, std::complex<double> squeezing_param) {
    ...
    static std::unique_ptr<Gate> Displacement(int qumode, std::complex<double> displacement_param) {
